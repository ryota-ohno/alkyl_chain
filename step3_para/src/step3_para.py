...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cy','cz','theta1','theta2','a','b','z','E','status','machine_type','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,max_2,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    fixed_param_keys = ['theta1','theta2','a','b','z'];opt_param_keys = ['cy','cz']
    ...
    for idx, row in df_prg_1.iterrows():
        ...
        if len(E_list1)!=7 :##get Eの長さは計算した分子の数
            continue
        else:
            len_prg_1-=1
            E1=float(E_list1[0]);E2=float(E_list1[1]);E3=float(E_list1[2]);E4=float(E_list1[3]);E5=float(E_list1[4])
            E6=float(E_list1[5]);E7=float(E_list1[6])
            E = E1 + E2 + E3 + E4 + E5 + E6 + E7
            df_E_1.loc[idx, ['E','E1','E2','E3','E4','E5','E6','E7','status']] = [E,E1,E2,E3,E4,E5,E6,E7,'Done']
...
def get_params_dict(auto_dir, num_nodes):
    ...
    for index in df_init_params_inprogress.index:##こちら側はinit_params内のある業に関する探索が終わった際の新しい行での探索を開始するもの ###ここを改良すればよさそう
        ...
        if isDone:
            opt_params_dict={'cy':np.round(opt_params_matrix[0][0],1),'cz':np.round(opt_params_matrix[0][1],1)
                            }
            ...
            for i in range(len(opt_params_matrix)):
                opt_params_dict={'cy':np.round(opt_params_matrix[i][0],1),'cz':np.round(opt_params_matrix[i][1],1)
                            }
    ...
    while True:
        ...
        for cy in [cy_init_prev-0.1,cy_init_prev,cy_init_prev+0.1]:
            for cz in [cz_init_prev-0.1,cz_init_prev,cz_init_prev+0.1]:
                ...
                df_val_xyz = df_val[(df_val['cy']==cy)&(df_val['cz']==cz)&
                                    (df_val['status']=='Done')]
                if len(df_val_xyz)==0:
                    para_list.append([cy,cz])
                    continue
                xyz_list.append([cy,cz]);E_list.append(df_val_xyz['E'].values[0])
        if len(para_list) != 0:
            return False,para_list
        cy_init,cz_init = xyz_list[np.argmin(np.array(E_list))]
        if cy_init==cy_init_prev and cz_init==cz_init_prev:
            return True,[[cy_init,cz_init]]
        else:
