...
import numpy as np
import random
from make_step2_auto_2 import exec_gjf
...
def listen(log_filepath_1,log_filepath_2,log_filepath_3):##args自体を引数に取るか中身をばらして取るかの違い    
    ...
    if len(E_list1)!=41 or len(E_list2)!=41 or len(E_list3)!=41:##計算する層状の分子数
        isOver =False
    else:
        isOver=True
    return isOver
...
def get_file_base_name(monomer_name,params_dict):
    a1_ = params_dict['a1']; a2_ = params_dict['a2']; b_ = params_dict['b']; theta = params_dict['theta']
    file_base_name = ''
    file_base_name += monomer_name
    file_base_name += '_step2_'
    file_base_name += 'a1={}_a2={}_b={}_theta={}_'.format(a1_,a2_,b_,theta)
...
def end_process(args):
    ...
    for i in range(len(E_list1)):
        z=rt_list[i];Et1=E_list1[i];Ep1=E_list2[i];Et4=E_list3[i]
