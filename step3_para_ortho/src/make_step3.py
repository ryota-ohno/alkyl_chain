...
MONOMER_LIST = ["C5",'C6']
############################汎用関数###########################
def get_monomer_xyzR(monomer_name,Ta,Tb,Tc,A1,A3):
    T_vec = np.array([Ta,Tb,Tc])
    df_mono=pd.read_csv('~/Working/alkyl_chain/step3_para_ortho/monomer/{}_re.csv'.format(monomer_name))
    ...
    xyz_array = np.matmul(xyz_array,Rod(ex,-A1).T)
    R_array = atoms_array_xyzR[:,3].reshape((-1,1))
    
    return np.concatenate([xyz_array,R_array],axis=1)
        
def get_monomer_xyzR_(monomer_name,Ta,Tb,Tc,A1,A3):
    T_vec = np.array([Ta,Tb,Tc])
    df_mono=pd.read_csv('~/Working/alkyl_chain/step3_para/monomer/{}_re.csv'.format(monomer_name))
    atoms_array_xyzR=df_mono[['X','Y','Z','R']].values
    ...
    xyz_array = np.matmul(xyz_array,Rod(ez,A3).T)
    xyz_array = np.matmul(xyz_array,Rod(ex,-A1).T)
    xyz_array = np.matmul(xyz_array,Rod(ez,180).T)
    xyz_array = xyz_array + T_vec
    R_array = atoms_array_xyzR[:,3].reshape((-1,1))
...
def make_xyzfile(monomer_name,params_dict):
    ...
    monomer_array_b2 = get_monomer_xyzR(monomer_name,0,-b,-2*z,A1,theta1)
    monomer_array_t1 = get_monomer_xyzR(monomer_name,a/2,b/2,z,A1,-theta1)
    monomer_array_t2 = get_monomer_xyzR(monomer_name,a/2,-b/2,-z,A1,-theta1)
    monomer_array_t3 = get_monomer_xyzR(monomer_name,-a/2,-b/2,-z,A1,-theta1)
    monomer_array_t4 = get_monomer_xyzR(monomer_name,-a/2,b/2,z,A1,-theta1)
    
    xyz_list=['400 \n','polyacene9 \n']##4分子のxyzファイルを作成
    
    monomers_array_4 = np.concatenate([monomer_array_i,monomer_array_0,monomer_array_b1,monomer_array_b2,monomer_array_t1,monomer_array_t2,monomer_array_t3,monomer_array_t4],axis=0)
...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,machine_type):
    ...
    dimer_array_b1 = np.concatenate([monomer_array_i,monomer_array_b1])
    dimer_array_b2 = np.concatenate([monomer_array_i,monomer_array_b2])
    ...
    file_description = f'{monomer_name}_a={a}_b={b}_z={z}_theta1={theta1}_theta2={theta2}_cy={cy}_cz={cz}'
    line_list_dimer_0 = get_xyzR_lines(dimer_array_0,file_description,machine_type)
    line_list_dimer_b1 = get_xyzR_lines(dimer_array_b1,file_description,machine_type)
    line_list_dimer_b2 = get_xyzR_lines(dimer_array_b2,file_description,machine_type)
    line_list_dimer_t1 = get_xyzR_lines(dimer_array_t1,file_description,machine_type)
    line_list_dimer_t2 = get_xyzR_lines(dimer_array_t2,file_description,machine_type)
    line_list_dimer_t3 = get_xyzR_lines(dimer_array_t3,file_description,machine_type)
    line_list_dimer_t4 = get_xyzR_lines(dimer_array_t4,file_description,machine_type)
