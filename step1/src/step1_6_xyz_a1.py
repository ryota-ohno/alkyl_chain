...
from make_6_xyz_a1 import exec_gjf##計算した点のxyzfileを出す
...
def listen(auto_dir,monomer_name,num_nodes,num_m2,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=1:##get Eの長さは計算した分子の数
            ...
            Et1=float(E_list[0])##8分子に向けてep1,ep2作成　ep1:b ep2:a
            E = Et1##エネルギーの値も変える
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_t1':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
