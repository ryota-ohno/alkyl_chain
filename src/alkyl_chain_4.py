...
from make_ac_3 import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['theta','a','b','phi1','phib','A1','A2','E','E_p1','E_t1','E_t2','E_m','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    maxnum_machine2 = 3#int(num_nodes/2) ##3にする
...
def get_params_dict(auto_dir, num_nodes):
    ...
    opt_param_keys = ['phi1']
    fixed_param_keys = ['a','b','theta','phib','A1','A2']
