...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['theta','a','b','phi1','A1','A2','E','E_p1','E_t1','E_t2','E_m','machine_type','status','file_name'])##いじる
...
def get_params_dict(auto_dir, num_nodes):
    ...
    fixed_param_keys = ['a','b','phi1','A1','A2']
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    a = init_params_dict['a']; b = init_params_dict['b']; phi1 = init_params_dict['phi1']
    while True:
        E_list=[];theta_list=[]
        for theta in [theta_init_prev-0.5,theta_init_prev,theta_init_prev+0.5]:
            ...
            theta_list.append(theta);E_list.append(df_val_phi1['E'].values[0])
