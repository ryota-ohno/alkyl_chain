...
def get_monomer_xyzR(monomer_name,Ta,Tb,Tc,A2,A3):
    ...
    df_mono=pd.read_csv('~/Working/alkyl_chain/dimer/monomer/{}_re.csv'.format(monomer_name))
...
def make_xyzfile(monomer_name,params_dict):
    theta = params_dict.get('theta',0.0); phi = params_dict.get('phi',0.0); r = params_dict.get('r',0.0); z = params_dict.get('z',0.0)
    x=r*np.cos(np.radians(phi));y=r*np.sin(np.radians(phi))
    monomer_array_1 = get_monomer_xyzR(monomer_name,0,0,0,0,theta)
    monomer_array_2 = get_monomer_xyzR(monomer_name,x,y,z,0,-theta)##1,2がb方向
...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,machine_type):
    ...
    file_description = f'{monomer_name}_theta={theta}_phi={phi}_r={r}_z={z}'
    line_list_dimer = get_xyzR_lines(dimer_array,file_description,machine_type)
