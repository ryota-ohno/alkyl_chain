...
def listen(auto_dir,monomer_name,num_nodes,max_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    fixed_param_keys = [];opt_param_keys = ['theta','phi','r','z']
...
def get_params_dict(auto_dir, num_nodes):
    ...
    for index in df_init_params_inprogress.index:##こちら側はinit_params内のある業に関する探索が終わった際の新しい行での探索を開始するもの ###ここを改良すればよさそう
        ...
        if isDone:
            opt_params_dict={'theta':np.round(opt_params_matrix[0][0],1),'phi':np.round(opt_params_matrix[0][1],1),'r':np.round(opt_params_matrix[0][2],1),'z':np.round(opt_params_matrix[0][3],1)}
            ...
            for i in range(len(opt_params_matrix)):
                opt_params_dict={'theta':np.round(opt_params_matrix[i][0],1),'phi':np.round(opt_params_matrix[i][1],1),'r':np.round(opt_params_matrix[i][2],1),'z':np.round(opt_params_matrix[i][3],1)}
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    theta_init_prev = init_params_dict['theta'];phi_init_prev = init_params_dict['phi']
    r_init_prev = init_params_dict['r'];z_init_prev = init_params_dict['z']
    while True:
        E_list=[];xyz_list=[]
        para_list=[]
        for theta in [theta_init_prev]:
            for phi in [phi_init_prev]:
                for r in [r_init_prev]:
                    for z in [z_init_prev]:
                        theta = np.round(theta);phi = np.round(phi);r = np.round(r,1);z = np.round(z,1)
                        df_val_xyz = df_val[(df_val['theta']==theta)&(df_val['phi']==phi)&
                                            (df_val['r']==r)&(df_val['z']==z)&(df_val['status']=='Done')]
                        if len(df_val_xyz)==0:
                            para_list.append([theta,phi,r,z])
                            continue
                        xyz_list.append([theta,phi,r,z]);E_list.append(df_val_xyz['E'].values[0])
        if len(para_list) != 0:
            return False,para_list
        theta_init,phi_init,r_init,z_init = xyz_list[np.argmin(np.array(E_list))]
        if theta_init==theta_init_prev and phi_init==phi_init_prev and r_init==r_init_prev and z_init==z_init_prev:
            return True,[[theta_init,phi_init,r_init,z_init]]
        else:
            theta_init_prev=theta_init;phi_init_prev=phi_init;r_init_prev=r_init;z_init_prev=z_init
