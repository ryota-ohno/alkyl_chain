...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['theta','a1','z1','a2','b2','z2','E','E1','E2','E3','E4','status'])##いじる
    ...
    if not os.path.exists(auto_csv_path2):        
        df_E_2 = pd.DataFrame(columns = ['theta','a2','b2','z2','E2','machine_type','status','file_name'])##いじる
    ...
    if not os.path.exists(auto_csv_path3):        
        df_E_3 = pd.DataFrame(columns = ['theta','a1','z1','a2','b2','z2','E3','E4','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,max_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    fixed_param_keys = ['theta'];opt_param_keys_1 = ['a1','z1'];opt_param_keys_2 = ['a2','b2','z2']
    ...
    for idx,row in zip(df_prg_1.index,df_prg_1.values):
        theta,a1,z1,machine_type = row##ここでx1~z2をdictにするのも有り?
    ...
    for idx, row in zip(df_prg_2.index, df_prg_2.values):
        theta,a2, b2, z2, machine_type = row  # Updated to a2, b2, z2
        file_name2 = f'{monomer_name}_theta={theta}_a2={a2}_b2={b2}_z2={z2}_2.log'  # Updated to file_name2 with a2, b2, z2
    ...
    for idx, row in zip(df_prg_3.index, df_prg_3.values):
        theta,a1,z1,a2,b2,z2,machine_type = row  # Updated to x3, y3, z3
        file_name3 = f'{monomer_name}_theta={theta}_a1={a1}_z1={z1}_a2={a2}_b2={b2}_z2={z2}_3.log'  # Updated to file_name3 with x3, y3, z3
...
def get_params_dict(auto_dir, num_nodes):
    ...
    for index in df_init_params_inprogress.index:##こちら側はinit_params内のある業に関する探索が終わった際の新しい行での探索を開始するもの ###ここを改良すればよさそう
        ...
        if isDone:
            opt_params_dict={'a1':np.round(opt_params_matrix[0][0],1),'z1':np.round(opt_params_matrix[0][1],1),
                            'a2':np.round(opt_params_matrix[0][2],1),'b2':np.round(opt_params_matrix[0][3],1),'z2':np.round(opt_params_matrix[0][4],1)
                            }
            ...
            for i in range(len(opt_params_matrix)):
                opt_params_dict={'a1':np.round(opt_params_matrix[i][0],1),'z1':np.round(opt_params_matrix[i][1],1),
                            'a2':np.round(opt_params_matrix[i][2],1),'b2':np.round(opt_params_matrix[i][3],1),'z2':np.round(opt_params_matrix[i][4],1)
                            }
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    a1_init_prev = init_params_dict['a1']; z1_init_prev = init_params_dict['z1']
    a2_init_prev = init_params_dict['a2'];b2_init_prev = init_params_dict['b2']; z2_init_prev = init_params_dict['z2']
    while True:
        E_list=[];xyz_list=[]
        para_list=[]
        for a1 in [a1_init_prev]:#-0.1,a1_init_prev,a1_init_prev+0.1]
            for z1 in [z1_init_prev]:
                for a2 in [a2_init_prev]:
                    for b2 in [b2_init_prev]:
                        for z2 in [z2_init_prev]:
                            a1 = np.round(a1,1);z1 = np.round(z1,1)
                            a2 = np.round(a2,1);b2 = np.round(b2,1);z2 = np.round(z2,1)
                            df_val_xyz = df_val[(df_val['a1']==a1)&(df_val['z1']==z1)&
                                                (df_val['a2']==a2)&(df_val['b2']==b2)&(df_val['z2']==z2)&
                                                (df_val['status']=='Done')]
                            if len(df_val_xyz)==0:
                                para_list.append([a1,z1,a2,b2,z2])
                                continue
        ...
        if a1_init==a1_init_prev and z1_init==z1_init_prev and a2_init==a2_init_prev and b2_init==b2_init_prev and z2_init==z2_init_prev:
            return True,[[a1_init,z1_init,a2_init,b2_init,z2_init]]
        else:
            a1_init_prev=a1_init;z1_init_prev=z1_init;a2_init_prev=a2_init;b2_init_prev=b2_init;z2_init_prev=z2_init
